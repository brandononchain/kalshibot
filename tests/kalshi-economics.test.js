...
test('expected value subtracts executable ask and fee', () => {
  assert.equal(expectedValuePerContract(0.6, 0.5, 0.02), 0.08);
  assert.equal(expectedValuePerContract(0.52, 0.5, 0.02), 0);
});

test('fractional Kelly uses contract price and fee, not probability-implied odds', () => {
  const fullKelly = kellyBankrollFraction(0.75, 0.65, 0, 1);
  assert.ok(Math.abs(fullKelly - (0.75 - 0.25 * 0.65 / 0.35)) < 1e-12);
